...
describe("shared production hardening", () => {
  ...
  it("persists and invalidates sessions through the storage interface", async () => {
    ...
    const session = { user: { id: "42", username: "tester" }, guildId: "guild", roleIds: ["member"], expiresAt: Date.now() + 60_000 };
  ...
  it("redacts secrets from development logs", () => {
    ...
    expect(output).toHaveBeenCalledWith("[info] test", { token: "[REDACTED]", route: "/api/test" });
  });

  it("does not expose unexpected server errors", () => {
    expect(getSafeError(new Error("database password is wrong"))).toEqual({ message: "Internal server error", statusCode: 500, code: "INTERNAL_ERROR" });
    expect(getSafeError(new AppError("Invalid input", 400, "INVALID_INPUT", true))).toEqual({ message: "Invalid input", statusCode: 400, code: "INVALID_INPUT" });
  });
});
