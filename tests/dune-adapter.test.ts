...
it("preserves the configured URL prefix and actor when requesting a linked player", async () => {
  ...
  expect(fetchMock).toHaveBeenCalledWith("https://console.test/prefix/api/integrations/discord/players/me", expect.objectContaining({
    method: "POST", body: JSON.stringify({ actor }), cache: "no-store",
