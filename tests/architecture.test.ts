...
const files = fs.readdirSync(root, { recursive: true }).map(String).filter(p => /\.tsx?$/.test(p)).map(p => path.join(root, p));
  ...
  it("keeps server modules out of the transitive browser import graph", () => {
    ...
    function visit(file: string) {
      ...
      for (const spec of imports(file)) {
        ...
        const resolved = [base, base + ".ts", base + ".tsx", path.join(base, "index.ts")].find(p => files.includes(p));
        if (resolved) visit(resolved);
      }
    }
    for (const file of files.filter(p => /^(pages\/(?!api\/)|instrumentation-client)/.test(normalize(p)))) visit(file);
  });
