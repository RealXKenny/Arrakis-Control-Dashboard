...
function visibleWorldRect(
  ...
  const mapWidth = config.width * zoom;
  const mapHeight = config.height * zoom;
  if (!(mapWidth > 0) || !(mapHeight > 0)) return null;
  const x0 = left / mapWidth;
  const x1 = (left + width) / mapWidth;
  const y0 = top / mapHeight;
  const y1 = (top + height) / mapHeight;
  const minY = config.minY + (config.maxY - config.minY) * (config.flipY ? 1 - y1 : y0);
  const maxY = config.minY + (config.maxY - config.minY) * (config.flipY ? 1 - y0 : y1);
  return {
    minX: config.minX + (config.maxX - config.minX) * x0,
    maxX: config.minX + (config.maxX - config.minX) * x1,
    minY,
    maxY,
