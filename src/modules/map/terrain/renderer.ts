import { BFS, BVS, FS, RFS, RVS, VS } from './shaders';
...
const LIFTON = 1.0;
const PATCHCUT = 400;
const PATCHFEATHER = 7000;
const PATCHCOL: [number, number, number] = [0.56, 0.442, 0.286];
const POICOL: [number, number, number] = [0.537, 0.659, 0.592];
const TILE = 64762;
const DETSTR = 0.6053;
const BRKTILE = 30000;
const BRKAMP = 420;
...
function compile(gl: WebGL2RenderingContext, type: number, source: string, label: string): WebGLShader {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    ...
    throw new Error(
      contextLost
        ? `graphics context lost while compiling ${label}`
        : `${label} failed to compile: ${log || 'browser provided no compiler details'}`,
    );
  }
  return shader;
}
function link(gl: WebGL2RenderingContext, vs: string, fs: string, label: string): WebGLProgram {
  ...
  gl.attachShader(program, compile(gl, gl.VERTEX_SHADER, vs, `${label} vertex shader`));
  gl.attachShader(program, compile(gl, gl.FRAGMENT_SHADER, fs, `${label} fragment shader`));
  gl.linkProgram(program);
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    throw new Error(`${label} program failed to link: ${gl.getProgramInfoLog(program)?.trim() || 'no linker details'}`);
  }
  return program;
}
export function createDeepDesertRenderer(canvas: HTMLCanvasElement, options: RendererOptions = {}): DeepDesertRenderer {
  ...
  // Dev note: when floats sink, the rocks still keep their composure.
  const rockWeight = floatBuffer ? ROCKW : 1.0;
  const terrain = link(gl, VS, FS, 'terrain');
  const resolve = link(gl, RVS, RFS, 'terrain resolve');
  const backdrop = link(gl, BVS, BFS, 'terrain backdrop');
  ...
  const t = {
    ...
    zhi: u(terrain, 'uZhi'),
    c: u(terrain, 'uC'),
    half: u(terrain, 'uHalf'),
    feather: u(terrain, 'uFeather'),
    hf: u(terrain, 'uHF'),
    hfMode: u(terrain, 'uHFMode'),
    hn: u(terrain, 'uHN'),
    hStep: u(terrain, 'uHStep'),
    hx0: u(terrain, 'uHX0'),
    hy0: u(terrain, 'uHY0'),
    hzlo: u(terrain, 'uHZlo'),
    hzhi: u(terrain, 'uHZhi'),
    wScale: u(terrain, 'uWScale'),
    bias: u(terrain, 'uBias'),
    lift: u(terrain, 'uLift'),
    patchCut: u(terrain, 'uPatchCut'),
    patchFeather: u(terrain, 'uPatchFeather'),
    patchCol: u(terrain, 'uPatchCol'),
    poiCol: u(terrain, 'uPoiCol'),
    brk: u(terrain, 'uBrk'),
    brkTile: u(terrain, 'uBrkTile'),
    brkAmp: u(terrain, 'uBrkAmp'),
    clipRaise: u(terrain, 'uClipRaise'),
    prepass: u(terrain, 'uPrepass'),
    view: u(terrain, 'uV'),
    d1: u(terrain, 'uD1'),
    d2: u(terrain, 'uD2'),
    tile: u(terrain, 'uTile'),
    detStr: u(terrain, 'uDetStr'),
    detail: u(terrain, 'uDetail'),
  };
  ...
  function uploadMipped(raw: Uint8Array, size: number): WebGLTexture {
    const expected = size * size * 4;
    if (raw.byteLength !== expected)
      throw new Error(`terrain texture is ${raw.byteLength} bytes, expected ${expected}`);
    const texture = gl.createTexture()!;
    gl.bindTexture(gl.TEXTURE_2D, texture);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA8, size, size, 0, gl.RGBA, gl.UNSIGNED_BYTE, raw);
  ...
  function ensureFramebuffer(width: number, height: number) {
    ...
    gl.texImage2D(
      gl.TEXTURE_2D,
      0,
      floatBuffer ? gl.RGBA16F : gl.RGBA8,
      width,
      height,
      0,
      gl.RGBA,
      floatBuffer ? gl.HALF_FLOAT : gl.UNSIGNED_BYTE,
      null,
    );
  ...
  function setAssets(shared: SharedAssets, layout: LayoutAssets) {
    ...
    if (sharedRef !== shared) {
      ...
      det1 = uploadMipped(shared.detail1, 1024);
      det2 = uploadMipped(shared.detail2, 1024);
      texBrk = uploadMipped(shared.breakup, 128);
      sharedRef = shared;
    }
    if (layoutRef !== layout) {
  ...
  return {
    ...
    dispose() {
      ...
      for (const program of [terrain, resolve, backdrop]) gl.deleteProgram(program);
      calls = [];
      sharedRef = null;
      layoutRef = null;
    },
