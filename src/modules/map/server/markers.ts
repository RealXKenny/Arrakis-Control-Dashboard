...
export function extractRows(data) {
  ...
  const candidates = [data.markers, data.rows, data.data?.markers, data.data?.rows, data.data];
...
export function filterMapMarkers(allMarkers, { playerId, playerName, playerData, session, playerBaseIds }) {
  ...
  return allMarkers.filter((marker) => {
    ...
    if (type === 'player') {
      return isCurrentPlayerMarker(marker, playerId, playerData, session);
    }
    ...
    if (type === 'base') {
      const markerBaseId = getMarkerBaseId(marker);

      return markerBaseId && playerBaseIds.has(markerBaseId);
    }
...
export function addOnlineStatus(markers, onlinePlayerIds) {
  return markers.map((marker) => {
    ...
    return {
      ...marker,
      online: markerPlayerId ? onlinePlayerIds.has(markerPlayerId) : false,
    };
  });
}
